...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		// XXX Implement this method in week 2
		 List<Integer> distance2_ = new ArrayList<Integer>();
//		 List<Integer> firstHop = new ArrayList<Integer>();
//		 firstHop = getNeighbors(v);
//		 try {
//		 for(int i : firstHop) {
//			 distance2_.addAll(getNeighbors(i));
//		 }
//		 }
...
//		 }
		...
		//square the matrix
		 ///  Can not do this!!!!  use 'for' loop
		//int [][] square_adjMatrix = adjMatrix;  
		int [][] square_adjMatrix = new int[getNumVertices()][getNumVertices()];
		for(int n = 0; n < getNumVertices(); n++) {
			for(int m = 0; m < getNumVertices(); m++) {
				square_adjMatrix[n][m] = 0;
				for(int l = 0; l < getNumVertices(); l++) {
					square_adjMatrix[n][m] =square_adjMatrix[n][m] + adjMatrix[n][l] * adjMatrix[l][m];
				}
			}
		}
		for(int k=0; k < getNumVertices(); k++) {
			for(int i= 0; i<square_adjMatrix[v][k] ; i++) {
				distance2_.add(k);
			}
		}
		//distance2_.add(18);
