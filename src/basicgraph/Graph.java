...
public abstract class Graph {
	 ...
	 */
	// getNeighbors(int v), getInNeighbors(int v), 
	public List<Integer> degreeSequence() {
		// XXX: Implement in part 1 of week 2
		List<Integer> degreeSequence = new ArrayList<Integer>();
		
				
		//get the degree of each vertex:  getNeighbors(i).size();
		//add to degreeSequence
		// sort from large to small
		
		int vertices_nums = getNumVertices();
		int t_degree = 0;
		for(int i = 0; i < vertices_nums; i++) {
			t_degree = getNeighbors(i).size() + getInNeighbors(i).size();
			degreeSequence.add(t_degree);
		}
		// Play around with sort, method 1
	...
	public static void main (String[] args) {
		...
		//GraphLoader.createIntersectionsFile("data/maps/NTU.map", "data/intersections/NTU.intersections");
